...
Map* MapBuilder::createMap(ofImage mapImage){
    ...
    for (int i = 0; i < mapImage.getWidth(); i++) {
        for (int j = 0; j < mapImage.getHeight(); j++) {
            ...
            if(currentPixel == boundBlock){
                ...
                PacMan = new Player(xPos,yPos,pixelMultiplier,pixelMultiplier, entityManager);
	...
	GhostSpawner *ghostSpawner = new GhostSpawner(505, 360);
	PacMan->setGhostSpawner(ghostSpawner);
